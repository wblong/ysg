 ...
 */
ol.control.Add = function(opt_options) {
    
    var options = opt_options || {};
    var _this = this;

    var controlDiv = document.createElement('div');
    controlDiv.className = 'ol-add ol-unselectable ol-control';
    ...
    this.setProperties({
        element: controlDiv,
        drawPoint:  new ol.control.Interaction({
...
ol.inherits(ol.control.Add, ol.control.Control);

ol.control.Add.prototype.setMap = function(map) {
    ...
    if(map!==null){
        ...
        if (layermanager!=null&&drawPoint.get('active')) {
                this.handleEvents = function(interaction, type) {
                    interaction.on('drawend', function (evt) {
                        var selectedLayer = layermanager.getLayer('realTimeLayerID');
        ...
        for (var j in headers) {
            ...
            td.addEventListener('click', function(evt){
                ...
                if (map instanceof ol.Editor) {
                    ...
                    overlay.show(feature.getGeometry().getCoordinates(),table); 
                }
...
ol.control.Measure = function (opt_options) {
    ...
    this.setProperties({
        ...
        measureLength: options.measureLength || new ol.control.Interaction({
            ...
            // tipLabel: options.measureLengthTipLabel || '长度',
            tipLabel:'长度',
        ...
        measureArea: options.measureArea || new ol.control.Interaction({
            ...
            // tipLabel: options.measureAreaTipLabel || '面积',
            tipLabel: '面积',
        ...
        measureAngle: options.measureAngle || new ol.control.Interaction({
            ...
            // tipLabel: options.measureAngleTipLabel || "角度",
            tipLabel:  "角度",
            ...
            switch (evt.type) {
                case 'pointermove':
                    ...
                    if (session === 'active') {
                        ...
                        this.get('tooltip').get('element').innerHTML = options.startLabel || '单击开始测量';
...
ol.control.Projection = function(opt_options) {
    ...
    projectionSelect.title = options.tipLabel || '设置投影';
...
ol.control.Rotation = function(opt_options) {
    ...
    rotationInput.title = options.tipLabel || '设置旋转';
...
ol.control.SelectionControls = function(opt_options) {
    ...
    removeButton.title = options.removeTipLabel || '清除选择';
    ...
    this.setProperties({
        ...
        selectSingle: options.selectSingle || new ol.control.Interaction({
            ...
            tipLabel: options.selectSingleTipLabel || '点选',
        ...
        selectMulti: options.selectMulti || new ol.control.Interaction({
            ...
            tipLabel: options.selectMultiTipLabel || '框选',
...
ol.control.ZoomToSelection = function(opt_options) {
    ...
    ol.control.ZoomTo.call(this, {
        ...
        tipLabel: options.tipLabel || '缩放至选择',
        label: options.label || ' ',
        extentFunction: function() {
            var map = _this.getMap();
        ...
        $.ajax({
            ...
            success: function(data){
                  if (layer) {
                    ...
                    //     if (layer.getSource().state==='ready') {
                    //         var features=layer.getSource().getFeatures();
                    //         features.forEach(function(feature){
                    //             feature.on('click',function(evt){
                    //                 //_this.getMap().getView().animate({ center:p });
                    //                 var overlay=_this.getMap().getOverlayById('1');
                    //                 var content = feature.getProperties();
                    //                 var table="<table class='ptable'>";
                    //                 Object.keys(content).forEach(function(key){
                    //                     if (key!=='geometry') {
...
ol.control.Route=function(opt_options){

    var options = opt_options || {};
    var _this = this;

    var controlDiv = document.createElement('div');
    controlDiv.className = options.className || 'ol-route ol-unselectable ol-control';
    
    var routeButton = document.createElement('button');
    routeButton.title = options.tipLabel || '运动轨迹';
    routeButton.textContent = options.label || '';

    var layer=null;
    

    routeButton.addEventListener("click",function(evt){
        if (layer) {
            ...
            layer.getSource().once('change',function(){
                if (layer.getSource().getState()==='ready') {
                    ...
                    features.forEach(function(feature){
                        var animating=false;
                        //routeCoords=feature.getGeometry().getCoordinates();
                        ...
                        feature.on('click',function(evt){
                            ...
                            var speed, now;
                            var geoMarker,startMarker,endMarker;
                            ...
                            var moveFeature=function(evt){
                                ...
                                vectorContext.drawFeature(feature,styles['car']);
                                _this.getMap().getView({center:currentPoint});
                                _this.getMap().render();
                            ...
                            var startAnimation=function(){
                                if (animating) {
                                    ...
                                    speed=1;
                                    _this.getMap().on('postcompose',moveFeature);
                                    _this.getMap().render();
                                }
                            };
                            var stopAnimation=function(ended){
                                ...
                                _this.getMap().un('postcompose',moveFeature);
                            };
                            layer.getSource().addFeatures([geoMarker,startMarker,endMarker]);
                            startAnimation();
                        });
                        // routeCoords.forEach(function(p){
...
ol.inherits(ol.control.Route,ol.control.Control);

ol.control.Route.prototype.setMap = function(map) {
    ol.control.Control.prototype.setMap.call(this,map);
    if (map===null) {
        ol.Observable.unByKey(this.get('chgEventId'));
    }else{
 ...
 * @param {olx.control.ControlOptions} options Control options.
 */
ol.control.Exit = function (opt_options) {
    ...
    exitButton.textContent = options.label || ' ';
    exitButton.setAttribute('id','exit');
    controlDiv.appendChild(exitButton);
